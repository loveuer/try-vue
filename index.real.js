class Vue {
    ...
    _toNewArray(f_name, arr) {
        ...
        window.eval(
            "var callback = function " +
                f_name +
                " (args, k){console.log('数组发生变化...');callback.vm.$data[k] = [...args]}"
        );
        ...
        Object.defineProperty(obj, key, {
            ...
            set: (newVal) => {
                ...
                if (newVal !== value) {
                    ...
                    value = Array.isArray(newVal)
                        ? this._toNewArray(key, newVal)
                        : newVal;
                    if (Number(key) >= 0) {
                        // 数组元素变化，需要深度调用一层的映射表中的watcher对象刷新
                        this._render[deep][key]["_renders"].forEach((watcher) =>
                            watcher.update()
                        );
                    } else {
                        // 平级元素变化，直接调用映射表中的watcher对象刷新
                        this._render[key]["_renders"].forEach((watcher) =>
                            watcher.update()
                        );
                    }
                }
    ...
    _parsing_v_click(curNode) {
        ...
        curNode.addEventListener("click", (e) => {
            ...
            if (template_for_id) {
                let vforVal = this._template[template_for_id].match(
                    /v-for=\"(.*?)\"/
                )[1]; // 得到v-for指令
                let [attrProps, attrName] = vforVal
                    .replace(/\s+/g, "")
                    .split("in"); // 解析x in y
                let indexKey = "",
                    valueKey = "";
                ...
                fn_params.forEach((param, idx) => {
                    ...
                    if (param === indexKey) {
                        fn_params[idx] = curNodeIdx;
                    }
                    if (param === valueKey) {
                        fn_params[idx] = this.$data[attrName][curNodeIdx];
                    }
                });
            }
    ...
    _parsing_v_for(curNode) {
        ...
        let replaces = {
            idx: indexKey ? indexKey : null,
            val: valueKey ? valueKey : attrProps,
        };
...
class Watcher {
    ...
    addBatchDom() {
        ...
        for (let i = 0; i < vmData.length; i++) {
            ...
            if (i >= 0) {
                _cloneNode.removeAttribute("v-for");
            }
    ...
    update_Array(vmData) {
        ...
        // 如果render属于数组项，则更新dom中的值
            ...
            let domStr = this.vm._template[
                this.dom.parentNode.getAttribute("v-data")
            ];
